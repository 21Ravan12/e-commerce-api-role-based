...
class PaymentProcessor {
  ...
  async process(order, additionalData = {}) {
    try {
      ...
      logger.info('Payment successfully processed'+ paymentRecord._id );
  ...
  try {
    ...
    // Process refund using the dedicated function
    const payment = await Payment.processRefund(order.paymentId, {
      amount: refundAmount,
      currency: 'USD',
