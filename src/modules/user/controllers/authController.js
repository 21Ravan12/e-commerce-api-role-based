...
const { registerSchema, loginSchema, completeRegistrationSchema, resetPasswordSchema, resendCodeSchema, verifyCodeSchema, newPasswordSchema } = require('../schemas/authSchemas');
...
class AuthController {
  ...
  async verifyResetCode(req, res) {
    try {
      ...
      if (resetData.code !== code.trim()) {
        // Update attempts in Redis if code is invalid
      ...
      // Mark the challenge as verified but don't delete it yet
      resetData.verified = true;
      resetData.verifiedAt = new Date().toISOString();
      
      // Store both the new token and updated challenge data
      await Promise.all([
        RedisClient.set(`pwd_token:${resetToken}`, JSON.stringify({
          userId: user._id.toString(),
          challenge,
        ...
        RedisClient.set(challenge, JSON.stringify(resetData), {
          EX: 900 // Maintain same expiration
        })
      ]);
  ...
  async oAuthCallback(req, res, next) {
    ...
    // First verify state parameter exists
    if (!req.query.state) {
      return res.status(400).json({ error: 'Missing state parameter' });
    }

    // Verify state token early
    try {
      ...
      return res.status(500).json({ error: 'State validation failed' });
    }

    // Now proceed with authentication
    passport.authenticate(provider, async (err, userData, info) => {
      try {
        // Error handling
        if (err) {
