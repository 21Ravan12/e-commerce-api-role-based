...
class ReturnRequestController {
  ...
  async createReturnRequest(req, res) {
    ...
    try {
      ...
      const order = await Order.getOrder(value.orderId, req.user._id);
      ...
      if (value.returnType === 'exchange') {
        ...
        const exchangeProduct = await Product.getProductById(value.exchangeProductId);
        if (!exchangeProduct) {
  ...
  async getReturnRequest(req, res) {
    ...
    try {
      ...
      const returnRequest = await ReturnRequest.getReturnRequest({id: req.params.id, userId: req.user._id, userRole: req.user.role});
